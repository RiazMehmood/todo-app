...
- [ ] T001 Verify Minikube is running and accessible (check `minikube status`)
- [ ] T002 Verify Dapr installed in cluster (check `kubectl get pods -n dapr-system`)
- [ ] T003 Verify Dapr components deployed (check kafka-pubsub, statestore, reminder-cron exist)
- [ ] T004 Verify Redpanda Cloud cluster accessible (check Redpanda Cloud console shows 4 topics)
- [ ] T005 Verify kafka-secrets exists in Kubernetes (check `kubectl get secret kafka-secrets`)
...
- [ ] T006 Create db-secrets in Kubernetes with DATABASE_URL from .env file
- [ ] T007 Create api-secrets in Kubernetes with BETTER_AUTH_SECRET from .env file
- [ ] T008 Create email-secrets in Kubernetes with placeholder sendgridApiKey (optional)
- [ ] T009 Verify all secrets created (db-secrets, api-secrets, email-secrets, kafka-secrets)

### Docker Image Builds

**Note**: Run `eval $(minikube docker-env)` before building to use Minikube's Docker daemon

- [ ] T010 [P] Build backend Docker image: `docker build -t todo-backend:latest ./backend`
- [ ] T011 [P] Build notification-service Docker image: `docker build -t todo-notification:latest ./services/notification-service`
- [ ] T012 [P] Build audit-service Docker image: `docker build -t todo-audit:latest ./services/audit-service`
- [ ] T013 Verify all images built successfully (check `docker images | grep todo`)
